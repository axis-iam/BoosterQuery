...
public class BoosterNativeExecutor {
    ...
    public <T> Page<T> queryPage(Pageable pageable, String sql, Map<String, Object> params, Class<T> resultType) {
        ...
        if (effectivePageable.isPaged()) {
            String countSql = SqlHelper.buildCountSql(normalizedSql);
        ...
        // 2. Data query
        String dataSql = SqlHelper.applySort(normalizedSql, effectivePageable.getSort());
    ...
    private long executeCount(String sql, Map<String, Object> params) {
        ...
        String countSql = normalizedSql.trim().toLowerCase().startsWith("select count")
                ? normalizedSql
                : SqlHelper.buildCountSql(normalizedSql);
    ...
    private int executeUpdate(String sql, Map<String, Object> params) {
        Query query = entityManager.createNativeQuery(SqlParameterCastNormalizer.normalize(sql));
        ParameterBinder.bind(query, params);
    ...
    private <T> List<T> executeQueryList(String sql, Map<String, Object> params, Class<T> resultType, Pageable pageable) {
        ...
        if (isEntity) {
            ...
            query = entityManager.createNativeQuery(normalizedSql, resultType);
        } else {
            // For DTOs or primitive types, use Tuple
            query = entityManager.createNativeQuery(normalizedSql, Tuple.class);
        }
