 ...
 */
package com.chaosguide.boosterquery.executor;

import com.chaosguide.boosterquery.cache.BoosterCache;
import com.chaosguide.boosterquery.config.BoosterQueryConfig;
import com.chaosguide.boosterquery.rewrite.BoosterSqlRewriter;
import com.chaosguide.boosterquery.support.JpaResultMapper;
import com.chaosguide.boosterquery.support.LimitAppender;
...
import com.chaosguide.boosterquery.support.SqlSanitizer;
...
public class BoosterQueryExecutor {
     ...
     * @throws com.chaosguide.boosterquery.rewrite.BoosterSqlRewriter.SqlRewriteException
    ...
    private PreparedQuery prepareQuery(String sql, Map<String, Object> params) {
        String normalizedSql = SqlParameterCastNormalizer.normalize(sql);
        if (normalizedSql == null || normalizedSql.isBlank()) {
            return new PreparedQuery(normalizedSql, params);
        }
        ...
        try {
            ...
            if (cache != null) {
                rewrittenSql = cache.get(new RewriteCacheKey(normalizedSql, nullParams));
            }
            ...
            if (rewrittenSql == null) {
                ...
                log.debug("SQL rewrite complete [MISS]:\n  Original SQL : {}\n  Rewritten SQL: {}\n  Removed params: {}",
                        SqlSanitizer.sanitize(normalizedSql), SqlSanitizer.sanitize(rewrittenSql), nullParams);

                if (cache != null) {
                    cache.put(new RewriteCacheKey(normalizedSql, nullParams), rewrittenSql);
                }
            ...
            log.error("SQL rewrite failed, aborting query to prevent errors. SQL (sanitized): {}", SqlSanitizer.sanitize(normalizedSql), e);
            throw e;
        }
    }
