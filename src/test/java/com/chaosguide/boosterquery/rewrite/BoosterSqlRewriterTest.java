package com.chaosguide.boosterquery.rewrite;
...
public class BoosterSqlRewriterTest {
    ...
    @Test
    void should_extractParamNameFromPostgresParamCast() {
    ...
    @Test
    void should_rewriteNullCondition_when_conditionContainsCast() {
        String sql = "SELECT * FROM global_identities WHERE app_id = :appId AND status = CAST(:status AS global_identity_status)";
        Map<String, Object> params = new HashMap<>();
        params.put("appId", "app-1");
        params.put("status", null);

        BoosterSqlRewriter.SqlRewriteResult result = BoosterSqlRewriter.rewrite(sql, params);

        String newSql = result.sql().toLowerCase(Locale.ROOT);
        assertTrue(newSql.contains("app_id = :appid"));
        assertFalse(newSql.contains(":status"));
        assertEquals("app-1", result.params().get("appId"));
        assertFalse(result.params().containsKey("status"));
    }

    @Test
    void should_normalizePostgresParamCast_beforeNullConditionRewrite() {
        String sql = "SELECT * FROM global_identities WHERE app_id = :appId AND status = :status::global_identity_status";
        Map<String, Object> params = new HashMap<>();
        params.put("appId", "app-1");
        params.put("status", null);
        ...
        assertTrue(newSql.contains("app_id = :appid"));
        assertFalse(newSql.contains(":status"));
        assertFalse(newSql.contains("::global_identity_status"));
    }
}
