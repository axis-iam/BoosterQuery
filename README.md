...
### PostgreSQL Named Parameter Casts

When a named parameter needs a PostgreSQL type cast, prefer the Hibernate-safe
`CAST(:param AS type)` form:

```sql
AND gi.status = CAST(:status AS global_identity_status)
```

BoosterQuery also normalizes PostgreSQL shorthand casts on named parameters before
creating the JPA query:
...
Column casts such as `created_at::date` are left unchanged. The `CAST(:param AS type)`
form remains the recommended cross-Hibernate style, especially for SQL shared outside
BoosterQuery.
...
BoosterQuery uses lightweight count/sort transformations for common pagination paths.
JSqlParser is loaded only when parser-based null-condition rewrite is actually needed.

If your test suite exercises JSqlParser while JaCoCo is enabled and JaCoCo fails on
generated parser methods such as `CCJSqlParserTokenManager.jjMoveNfa_0`, exclude the
JSqlParser package from test instrumentation:
...
logging:
  level:
    com.chaosguide.boosterquery: WARN  # Disable DEBUG logs
```
