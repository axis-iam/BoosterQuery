...
每个逻辑阶段先运行相关目标测试；提交前再运行 `./gradlew build`，避免重复执行整套测试。
...
以下场景 MUST 先完成方案规划，产出方案并等待用户确认后再编码：
...
|-------|-------|
| `rewrite` | SQL 改写管线 |
| `executor` | BoosterNativeExecutor / BoosterQueryExecutor |
