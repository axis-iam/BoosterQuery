...
- 覆盖对象：`JSqlParserRewriter`、`BoosterSqlRewriter`、`JpaResultMapper`、`ParameterBinder`、`LimitAppender`
...
- 工具：`@SpringBootTest` + TestContainers（MySQL 8.0.33 + PostgreSQL 17）
- 验证端到端流程：`@BoosterQuery` → SQL 改写 → 参数绑定 → 结果映射
- 每个集成测试类使用 `@Testcontainers(disabledWithoutDocker = true)`，Docker 不可用时自动跳过
...
**DML 安全边界：**
- [ ] 直接调用 `JSqlParserRewriter` 时，UPDATE/DELETE 全条件移除 → 应抛出 `IllegalStateException`
- [ ] 通过 `BoosterSqlRewriter` 调用时，上述异常 → 应包装为 `BoosterSqlRewriter.SqlRewriteException`
- [ ] UPDATE/DELETE 至少保留一个条件 → 正常执行

### 3. 异常路径
- 非法 SQL 语法 → 验证异常类型和消息
- JSqlParser 无法解析的方言 → 验证明确抛出 `BoosterSqlRewriter.SqlRewriteException`，不得降级执行原 SQL
...
- [ ] DML 变更：`JSqlParserRewriterTest` 与 `BoosterSqlRewriterTest` 中安全拦截用例已覆盖
