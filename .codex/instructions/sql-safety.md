...
```java
// Sort 参数必须通过 SqlHelper.applySort() 的属性格式校验
...
底层 AST 改写必须抛出 `IllegalStateException`，上层 `BoosterSqlRewriter` 再包装为
`BoosterSqlRewriter.SqlRewriteException`，绝不执行全表操作。

```java
// JSqlParserRewriter 中的安全检查（不得删除或绕过）
if (原SQL有WHERE条件 && 改写后WHERE条件全部消失) {
    throw new IllegalStateException(
        "All WHERE conditions removed; refusing full-table operation. SQL (sanitized): "
...
try {
    ...
    throw new BoosterSqlRewriter.SqlRewriteException("SQL rewrite failed: " + e.getMessage(), e);
}
...
- `LimitAppender` 仅用于检测已有 `LIMIT`；实际自动限制通过 JPA `query.setMaxResults()` 完成
...
- `BoosterQueryConfig` / `BoosterQueryProperties` 的配置属性 key（`booster.query.*`）一旦发布不得重命名
